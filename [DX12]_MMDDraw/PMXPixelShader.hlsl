...
float4 main(Model input) : SV_TARGET
{
    ...
    float4 ambCol = float4(ambient * 0.1, 1); //アンビエント

  //  return float4((diffuse.rgb * texColor.rgb * toonDif.rgb + saturate(float3(specularB * specular.rgb))), texColor.a * diffuse.a);
